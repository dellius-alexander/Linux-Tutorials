...
2. Extract the `docker-credential-pass` binary to `/usr/local/bin` or `~/.local/bin`. You must add the binary to `$PATH`.
    ...
    Run `docker-credential-pass (store|get|erase|list|version)` to verify the binary is working correctly.  you should see:
<br/>
...
6. Initialize you password store on Clent and Server:
    ...
    # Initialize a git repo for your password store
    $ pass git init
    # To track a remote password-store on a server
    SERVER $ git init --bare ~/.password-store
    # On Client, add the remote password store and provide it a name
